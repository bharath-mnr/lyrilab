...
// src/App.jsx - Updated with SEO Manager
import React, { Suspense, lazy } from 'react';
import { BrowserRouter as Router, Routes, Route } from 'react-router-dom';
import { AudioProvider } from './contexts/AudioContext';
import Navbar from './layout/Navbar';
...
const preloadedComponents = Object.fromEntries(
  ALL_TOOLS.map(tool => [
    ...
    lazy(() => {
      ...
      return moduleLoader().catch(error => {
        console.error(`Failed to load component for ${tool.component}:`, error);
        return {
          default: () => (
            <div className="flex items-center justify-center min-h-screen bg-red-900 text-white text-xl p-4 text-center">
              <p>Error loading {tool.name || tool.component} page. Please try again later.</p>
...
// SEO-enhanced component wrapper
const SEOWrappedComponent = ({ Component, toolData, pageId, ...props }) => {
  return (
    <SEOHead pageId={pageId} toolData={toolData}>
      <Component {...props} />
    </SEOHead>
  );
};
          ...
          <main className="flex-grow">
            <Suspense fallback={<LoadingFallback />}>
              <Routes>
                ...
                {/* Tool Routes with SEO */}
                {toolsWithoutHome.map(tool => {
                  const LazyComponent = preloadedComponents[tool.component];

                  if (!LazyComponent) {
                    console.error(`Error: Lazy component "${tool.component}" not found`);
                    return null;
                  }
                ...
                <Route 
                  ...
                  element={
                    <SEOHead pageId="terms-and-conditions">
